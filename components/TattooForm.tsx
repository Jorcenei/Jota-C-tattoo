...
export const TattooForm: React.FC<TattooFormProps> = ({ onGenerate, isGenerating, initialReferenceImages }) => {
  ...
  // Load initial reference images if provided
  useEffect(() => {
    ...
    <form onSubmit={handleSubmit} className="w-full max-w-xl flex flex-col gap-6 bg-zinc-900/50 p-6 rounded-2xl border border-zinc-800/50 backdrop-blur-sm">
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          className="w-full h-32 bg-zinc-950 border border-zinc-700 rounded-xl p-4 text-zinc-100 placeholder-zinc-600 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none resize-none transition-all"
          required
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-col gap-3">
          <div 
            ...
            className="border-2 border-dashed border-zinc-700 hover:border-amber-500/50 hover:bg-zinc-900/50 rounded-xl p-4 transition-all cursor-pointer flex items-center justify-center gap-2 text-zinc-500"
          >
          ...
          {referenceImages.length > 0 && (
            <div className="grid grid-cols-3 gap-2">
              {referenceImages.map((img, idx) => (
                <div key={idx} className="relative aspect-square bg-zinc-950 rounded-lg overflow-hidden border border-zinc-700 group">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <select
              ...
              className="w-full bg-zinc-950 border border-zinc-700 rounded-xl p-3 text-zinc-100 appearance-none focus:ring-2 focus:ring-amber-500 outline-none cursor-pointer"
            >
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => setOrientation(TattooOrientation.Vertical)}
              className={`flex-1 p-3 rounded-xl border flex items-center justify-center gap-2 transition-all ${orientation === TattooOrientation.Vertical ? 'bg-zinc-800 border-amber-500 text-amber-200' : 'bg-zinc-950 border-zinc-700 text-zinc-400'}`}
            >
              <RectangleVertical className="w-4 h-4" />
            </button>
            <button
              type="button"
              onClick={() => setOrientation(TattooOrientation.Horizontal)}
              className={`flex-1 p-3 rounded-xl border flex items-center justify-center gap-2 transition-all ${orientation === TattooOrientation.Horizontal ? 'bg-zinc-800 border-amber-500 text-amber-200' : 'bg-zinc-950 border-zinc-700 text-zinc-400'}`}
            >
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
            ...
            <button
              ...
              className={`w-full py-2.5 px-3 rounded-xl border flex items-center justify-between transition-all ${includeBackground ? 'bg-zinc-800 border-amber-500 text-amber-200' : 'bg-zinc-950 border-zinc-700 text-zinc-400'}`}
            >
              <span className="text-sm">{includeBackground ? 'Temático/Artístico' : 'Neutro/Limpo'}</span>
              <div className={`w-10 h-5 rounded-full relative transition-colors ${includeBackground ? 'bg-amber-500' : 'bg-zinc-700'}`}>
                <div className={`absolute top-1 left-1 w-3 h-3 bg-white rounded-full transition-transform ${includeBackground ? 'translate-x-5' : 'translate-x-0'}`} />
              </div>
            </button>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex p-1 bg-zinc-950 rounded-xl border border-zinc-800">
            <button
              ...
              className={`flex-1 py-1.5 text-sm font-medium rounded-lg transition-all ${numberOfImages === 1 ? 'bg-zinc-800 text-white shadow-md' : 'text-zinc-500 hover:text-zinc-300'}`}
            >
              1
            </button>
            <button
              ...
              className={`flex-1 py-1.5 text-sm font-medium rounded-lg transition-all ${numberOfImages === 2 ? 'bg-zinc-800 text-white shadow-md' : 'text-zinc-500 hover:text-zinc-300'}`}
            >
              2
            </button>
          </div>
      ...
      >
        {isGenerating ? (
          <>
            ...
            {referenceImages.length > 0 ? 'Processando Imagens...' : 'Criando Arte...'}
          </>
        ) : (
          <>
